...
"""
        ...
        # Get the output
        logs = await self._sandbox.process.get_session_command_logs(self._session_id, response.cmd_id)

        if response.exit_code != 0:
            self.logger.error(f"HTTP request failed with exit code {response.exit_code}: {logs.stderr}")
            msg = f"Failed to call swerex API: {logs.stderr}"
        ...
        try:
            ...
            msg = f"Invalid JSON response from swerex: {logs.stdout}"
            raise RuntimeError(msg) from e

        # Check for errors
        if "error" in result:
            msg = f"Request failed: {result['error']}"
            raise RuntimeError(msg)
        ...
        if status >= 400:
            try:
                ...
                if "swerexception" in error_data:
                    ...
                    msg = f"swerex exception: {exc_info.get('message', 'Unknown error')}"
                    raise RuntimeError(msg)
                msg = f"HTTP {status}: {error_data}"
                raise RuntimeError(msg)
        ...
        try:
            ...
            msg = f"Invalid JSON response body: {body}"
            raise RuntimeError(msg) from e
        ...
        try:
            ...
            if response.exit_code == 0:
                ...
                return IsAliveResponse(
                    is_alive=False, message=f"Health check failed with exit code {response.exit_code}"
                )
        except Exception as e:
    ...
    async def upload(self, request: UploadRequest) -> UploadResponse:
        ...
        if response.exit_code != 0:
            logs = await self._sandbox.process.get_session_command_logs(self._session_id, response.cmd_id)
            msg = f"Upload failed: {logs.stderr}"
            raise RuntimeError(msg)
