...
async def test_write_to_non_existent_directory(remote_runtime: RemoteRuntime, tmp_path: Path):
    ...
    response = await remote_runtime.write_file(WriteFileRequest(path=str(non_existent_dir), content="test"))
...
async def test_multiple_isolated_shells(remote_runtime: RemoteRuntime):
    shell1 = await remote_runtime.create_session(CreateSessionRequest(session="shell1"))
    shell2 = await remote_runtime.create_session(CreateSessionRequest(session="shell2"))
...
async def test_empty_command(remote_runtime: RemoteRuntime):
    r = await remote_runtime.execute(C(command="", shell=True))
    assert r.success
    r = await remote_runtime.execute(C(command="\n", shell=True))
    assert r.success
...
async def test_upload_file(runtime_with_default_session: RemoteRuntime, tmp_path: Path):
    ...
    r = await runtime_with_default_session.upload(
        UploadRequest(source_path=str(file_path), target_path=str(tmp_target))
    )
    assert r.success
...
async def test_upload_directory(runtime_with_default_session: RemoteRuntime, tmp_path: Path):
    ...
    r = await runtime_with_default_session.upload(UploadRequest(source_path=str(dir_path), target_path=str(tmp_target)))
    assert r.success
    assert (
